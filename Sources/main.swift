...
if args.count >= 3 && !args.contains("--config") {
  ...
  if let configIndex = args.firstIndex(of: "--config"), configIndex + 1 < args.count {
    ...
    // Default to stringsGen.yml if no --config is provided
    configPath = "stringsGen.yml"
  }

  // Verify the config file exists
  guard FileManager.default.fileExists(atPath: configPath) else {
    print("Error: No parameters provided and config file '\(configPath)' not found.")
    print("Usage: swift stringsGen.swift <xcstringsPath> <outputFilePath>")
    print("   or: swift stringsGen.swift --config <configFile.yml>")
    exit(1)
  }
...
/// Automatically generated by `stringsGen.swift`. Do not edit manually.
///
